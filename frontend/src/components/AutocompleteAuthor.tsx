...
import styles from "./AutocompleteAuthor.module.scss";

interface AutocompleteAuthorProps {
   onAuthorChange?: (author: { id: string; name: string }) => void;
}

const AutocompleteAuthor: React.FC<AutocompleteAuthorProps> = ({ onAuthorChange }) => {
   ...
   return (
      <form className={styles.authorForm}>
         ...
         {selectedAuthorId && <p className={styles.authorSelectedP}>Selected Author: {selectedAuthorId}</p>}
      </form>
   );
};
